...
void Comp::swipe(int laneIndex, double startBeat, double endBeat)
{
    if (startBeat >= endBeat) return;

    // Step 1: Remove/trim any existing segments that overlap [startBeat, endBeat)
    std::vector<CompSegment> newSegs;
    for (auto& seg : segments)
    {
        if (seg.endBeat <= startBeat || seg.startBeat >= endBeat)
        {
        ...
        else
        {
            ...
            if (seg.startBeat < startBeat)
            {
                ...
                CompSegment left;
                left.laneIndex = seg.laneIndex;
                left.startBeat = seg.startBeat;
                left.endBeat   = startBeat;
                newSegs.push_back(left);
            }
            if (seg.endBeat > endBeat)
            {
                // Right portion survives
                CompSegment right;
                right.laneIndex = seg.laneIndex;
                right.startBeat = endBeat;
                right.endBeat   = seg.endBeat;
                newSegs.push_back(right);
            }
            // The middle portion [max(seg.start, startBeat), min(seg.end, endBeat)) is removed
        }
    }

    // Step 2: Add the new segment
    CompSegment newSeg;
    newSeg.laneIndex = laneIndex;
    newSeg.startBeat = startBeat;
    newSeg.endBeat   = endBeat;
...
int Comp::activeLaneAtBeat(double beat) const
{
    for (auto& seg : segments)
    {
    ...
    return -1;  // gap — no lane active
}

void Comp::sortAndMerge()
{
    // Sort by startBeat
    ...
    // Merge adjacent segments on the same lane
    ...
    for (size_t i = 1; i < segments.size(); ++i)
    {
        ...
        if (cur.laneIndex == prev.laneIndex &&
            std::abs(cur.startBeat - prev.endBeat) < 0.001)
        {
