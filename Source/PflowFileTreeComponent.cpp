...
//==============================================================================
class PflowFileListTreeItem final : public TreeViewItem,
                               private TimeSliceClient,
                               private AsyncUpdater
{
public:
    PflowFileListTreeItem (PflowFileTreeComponent& treeComp,
                      const File& f,
                      TimeSliceThread& t)
        : file (f),
          owner (treeComp),
    ...
    ~PflowFileListTreeItem() override
    {
    ...
    {
        const Font f (FontOptions ((float) metrics::browserFontSize));
        const int tw = roundToInt (std::ceil (f.getStringWidthFloat (file.getFileName())));
        return 32 + tw + 32;
    }
...
private:
    ...
    {
        ignoreUnused (onlyUpdateIfCached);
        if (icon.isNull())
        {
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (PflowFileListTreeItem)
};
...
class PflowFileTreeComponent::Controller final : private DirectoryScanner::Listener
{
public:
    explicit Controller (PflowFileTreeComponent& ownerIn)
        : owner (ownerIn),
...
private:
    ...
    static void forEachItemRecursive (TreeViewItem* item, ItemCallback&& cb)
    {
        ...
        if (auto* fileListItem = dynamic_cast<PflowFileListTreeItem*> (item))
            cb (fileListItem);
    ...
    {
        auto* parentItem = [&]() -> PflowFileListTreeItem*
        {
        ...
        {
            auto* fileItem = dynamic_cast<PflowFileListTreeItem*> (parentItem->getSubItem (i));
        ...
        struct Comparator
        {
            ...
            {
                auto* item1 = dynamic_cast<PflowFileListTreeItem*> (first);
                auto* item2 = dynamic_cast<PflowFileListTreeItem*> (second);
    ...
    std::unique_ptr<PflowFileListTreeItem> createNewItem (const File& file)
    {
        auto newItem = std::make_unique<PflowFileListTreeItem> (owner,
                                                           file,
    ...
    PflowFileTreeComponent& owner;
    std::map<File, PflowFileListTreeItem*> treeItemForFile;
    DirectoryScanner scanner;
    std::optional<File> pendingFileSelection;
};

//==============================================================================
PflowFileTreeComponent::PflowFileTreeComponent (DirectoryContentsList& listToShow)
...
PflowFileTreeComponent::~PflowFileTreeComponent()
{
    deleteRootItem();
}

void PflowFileTreeComponent::refresh()
{
    controller->refresh();
}

//==============================================================================
File PflowFileTreeComponent::getSelectedFile (const int index) const
{
    if (auto* item = dynamic_cast<const PflowFileListTreeItem*> (getSelectedItem (index)))
        return item->file;

    return {};
}

void PflowFileTreeComponent::deselectAllFiles()
{
    clearSelectedItems();
}

void PflowFileTreeComponent::scrollToTop()
{
    getViewport()->getVerticalScrollBar().setCurrentRangeStart (0);
}

void PflowFileTreeComponent::setDragAndDropDescription (const String& description)
{
    dragAndDropDescription = description;
}

void PflowFileTreeComponent::setSelectedFile (const File& target)
{
    controller->selectFile (target);
}

void PflowFileTreeComponent::setItemHeight (int newHeight)
{
